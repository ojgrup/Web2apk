...
apk() {
    ...
    if [ "$build_task" = "bundleRelease" ]; then
        # Periksa AAB yang baru dibuat (menggunakan wildcard untuk menangani nama file)
        ...
        if [ -n "$built_aab_path" ]; then
            ...
            # Salin ke root untuk log yang lebih jelas, meskipun artifact di-upload dari built_aab_path
        ...
        # Log untuk build APK rilis
        log "$build_task completed (Output not copied)"
    else
        # Log untuk build debug atau lainnya
        log "$build_task completed (Output not copied)"
    fi
}
...
# --- FUNGSI BUILD YANG DIPERBAIKI ---
build() {
    apply_config $@
    # Panggil fungsi apk dengan task 'bundleRelease' (untuk AAB)
    apk bundleRelease
}
# --- AKHIR FUNGSI BUILD YANG DIPERBAIKI ---

# --- FUNGSI DEBUG YANG DIPERBAIKI ---
debug() {
    apply_config $@
...
# --- AKHIR FUNGSI DEBUG YANG DIPERBAIKI ---
